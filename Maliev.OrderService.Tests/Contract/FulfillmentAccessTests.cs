...
[Collection("Database")]
public class FulfillmentAccessTests : IClassFixture<TestWebApplicationFactory>
{
    ...
    public FulfillmentAccessTests(TestWebApplicationFactory factory)
    {
        _factory = factory;
    }
    ...
    {
        // Arrange
        var client = _factory.CreateAuthenticatedClient("fulfillment-user",
        ...
        // Act
        var response = await client.PostAsJsonAsync($"/order/v1/orders/{orderId}/statuses", request);
    ...
    public async Task Fulfillment_CannotUpdatePricing()
    {
        ...
        // Act
        var response = await client.PutAsJsonAsync($"/order/v1/orders/{orderId}", request);
