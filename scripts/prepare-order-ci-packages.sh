...
# These repositories are checked out beneath the service workspace in CI. Isolate them from
# OrderService's root editor configuration so their immutable source builds exactly as it does
# at each repository root, without inheriting host-repository analyzer policy.
